...
class StartScreen(tk.Frame):
    def __init__(self, master):
        ...
        try:
            imdir = os.path.join(script_dir, "newlogoupsacled 2.png")
            logo_image = Image.open(imdir)
...
class VideoPlayer(tk.Frame):
    def __init__(self, master):
        ...
        self.case_number, self.timestamp = self.create_new_case_folder()
        self.case_folder = os.path.join(script_dir,'..',"Case_Files", f"Case_{self.case_number}_{self.timestamp}")
        os.makedirs(self.case_folder, exist_ok=True)
    ...
    def create_new_case_folder(self):
            ...
            case_folder_temp = os.path.join(script_dir,'..',"Case_Files")
            if not os.path.exists(case_folder_temp):
                os.makedirs(case_folder_temp)

            # Get the highest case number to determine the next available case number
            case_folders = [d for d in os.listdir(case_folder_temp) if os.path.isdir(os.path.join(case_folder_temp, d))]
    ...
    def run_autossh(self):
        try:
            ...
            autossh_path = os.path.join(script_dir, "autossh.py")
    ...
    def open_system_logs(self):
        try:
            system_log_path = os.path.join(script_dir, "arthrex_system_logs.txt")
