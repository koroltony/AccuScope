...
class ErrorDetectionApp:
    ...
    def create_widgets(self):
        ...
        # **Bind double-click to play the ±2 s clip**
        self.error_list.bind('<Double-1>', self.on_error_doubleclick)
    ...
    def on_error_doubleclick(self, event):
        sel = self.error_list.selection()
        if not sel:
            return
        ts_str = self.error_list.item(sel[0], "values")[0]  # "HH:MM:SS"
        h, m, s = ts_str.split(":")
        t = int(h)*3600 + int(m)*60 + int(s)
        self.show_error_clip(t)

    def show_error_clip(self, error_time):
        if not self.video_path:
            return

        cap = cv2.VideoCapture(self.video_path)
        start_t = max(0, error_time - 2)
        end_t   = min(self.video_length, error_time + 2)
        start_f = int(start_t * self.fps)
        end_f   = int(end_t * self.fps)
        cap.set(cv2.CAP_PROP_POS_FRAMES, start_f)

        clip_win = tk.Toplevel(self.root)
        clip_win.title(f"Clip around {self.convert_seconds_to_hh_mm_ss(error_time)}")
        clip_label = tk.Label(clip_win)
        clip_label.pack()
        ...
        def play():
            ...
            resized = cv2.resize(rgb, (400, 240))
            img = ImageTk.PhotoImage(Image.fromarray(resized))
            clip_label.config(image=img)
            clip_label.image = img
            current += 1
            clip_win.after(int(1000 / self.fps), play)
